/**
 * v5.2.11 — PM Clearance Return deadlock vs live PM Request available balance.
...
const SCREEN = path.join(__dirname, "screenshots", "pm_clearance_return_funding_v5211");
const BASE =
  process.env.FRAPPE_E2E_BASE_URL || "http://development.localhost:8000";
const PREP =
  "erpnext_extensions.petty_management.e2e.pm_clearance_return_funding_availability_v5211_prep";
...
async function main() {
  const fixtures = prep("prepare_v5211_funding_deadlock_fixtures");
  ...
  try {
    ...
    const available = fixtures.available;
    const pwd = fixtures.password;

    // Finance reviewer: Approve must fail; Return must pass
    ...
    assert(!approveFail.ok, "Finance Approve must fail while over-allocated vs request");
    ...
    if (approveFail.error && approveFail.error !== "[object Object]") {
      assert(
        /available|exceed|balance|over.?alloc/i.test(approveFail.error),
        `expected availability error, got: ${approveFail.error}`
      );
    }
    ...
    assert(afterReturn.name === name, "document name must be unchanged");
    assert(afterReturn.workflow_state === "Draft", `expected Draft, got ${afterReturn.workflow_state}`);
    assert(
      (afterReturn._assign || []).includes(fixtures.users.holder.email) ||
        afterReturn.status === "Draft",
      "expected Draft with holder path after Return"
    );
    await context.close();

    // Holder corrects allocation to live available
    ...
    assert(Math.abs(fixed.allocated_amount - available) < 0.01, "allocation not fixed");
    ...
    assert(finalSnap.name === name, "same document name through end");
